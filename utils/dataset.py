...
class IXIdataset(Dataset):
    def __init__(self, data_dir, args, validtion_flag=False):
        ...
        self.masks = loadmat(mask_path)['mask1']
...
##################################################################################################################
        # mask = self.mask_func([256,256,1], seed=self.args.seed)
        # mask = mask.numpy()
        #
        # mask = np.append(mask, mask, axis=0).squeeze()
        #
        # maskedNot = 1 - mask
#####################################################################################################################
        # masked_Kspace, mask, _ = apply_mask(torch.from_numpy(kspace), self.mask_func, 123)        #fastmri random mask generate
