...
class netLoss():
    ...
    def __init__(self, args, masked_kspace_flag=True):
        ...
        #         except:
        #             self.mask = torch.tensor(masks_dictionary['population_matrix'] == 1, device=

        mask_path='./Masks_new/{}/{}.mat'.format(args.mask_type, args.accelerations)
        masks_dictionary = loadmat(mask_path)
        self.mask = torch.tensor(masks_dictionary['mask1'] == 1, device=self.args.device)
    ...
    def calc_gen_loss(self, pred_Im, pred_K, tar_Im, tar_K,D_fake=None):
        ...
        if D_fake is not None:
            ...
            advLoss = 0
        fullLoss = self.ImL2_weights*ImL2 + self.ImL1_weights*ImL1 + self.KspaceL2_weights*KspaceL2 + self.AdverLoss_weight*advLoss + self.FFLLoss_weight
