...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        patch_size = to_2tuple(7)

        self.img_size = img_size
        self.patch_size = patch_size
        self.H, self.W = img_size[0] // stride, img_size[1] // stride
        self.num_patches = self.H * self.W
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride,       #(32,32,16,4,8)-->(4,32,65,65)  #(32,32,7,4,8)-->
...
class VicinityVisionTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=32, in_chans=256, num_classes=1, embed_dims=[64, 128, 256, 512],
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            outs.append(x)
        # x = x.reshape(B, 128, 128, -1).permute(0, 3, 1, 2).contiguous()
        return x.mean(dim=1), outs
...
def vvt_tiny(pretrained=False, **kwargs):
    model = VicinityVisionTransformer(
        ...
        patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[1,1,1,1], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
