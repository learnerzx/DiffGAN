...
if __name__ == "__main__":
    ...
    for i, infile in enumerate(test_files):
        ...
        with h5py.File(infile, 'r') as f:
            ...
            fully_sampled_imgs = f['data'][35:115,:,:,0].squeeze()

            fully_sampled_imgs=fully_sampled_imgs.transpose((1, 2, 0))
        #Preprocess data:
        ...
        for slice_num in range(0,80):
            add = int(args.num_input_slices / 2)
            with h5py.File(infile, 'r') as f:
                if slice_num == 0:
                    imgs = np.dstack((fully_sampled_imgs[:, :, 0], fully_sampled_imgs[:, :, 0], fully_sampled_imgs[:, :, 1]))
                elif slice_num == img_shape[2]-1:
                    imgs = np.dstack((fully_sampled_imgs[:, :, slice_num-1], fully_sampled_imgs[:, :, slice_num], fully_sampled_imgs[:, :, slice_num]))
                else:
                    imgs = np.dstack((fully_sampled_imgs[:, :, slice_num-1], fully_sampled_imgs[:, :, slice_num], fully_sampled_imgs[:, :, slice_num + 1]))
