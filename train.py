...
def train(args):
    ...
    parameters=get_parameter_number(G_model)
    logging.info(f'Generator parameters: {parameters["Total"]}')
    logging.info(f'Generator trainable parameters: {parameters["Trainable"]}')
    # Init Dataloaders
    if args.dataset == 'IXI':
            ...
            with tqdm(desc=f'Epoch {epoch + 1}/{args.epochs_n}', unit=' imgs') as pbar:
                ...
                for batch in train_loader:
                    ...
                    if args.GAN_training:
                        ...
                        # D_real_pred = D_model(Diffusion(real_D_example))
                        # D_fake_pred = D_model(Diffusion(fake_D_example))
                        if args.GP:
                    ...
                    if args.GAN_training:
                        ...
                        if train_D:
                            ...
                            D_optimizer.step()
                    #
                    # if args.update_diffusion:  # batch_idx % ada_interval == 0
                    # adjust = np.sign(sign(D_model(real_images)) - ada_target) * C  # C = (batch_size * ada_interval) / (ada_kimg * 1000)
                    ...
                    if args.GAN_training:
                        pbar.set_postfix(**{'FullLoss': FullLoss.item(),'ImL2': ImL2.item(), 'ImL1': ImL1.item(),
                                            'KspaceL2': KspaceL2.item()*args.loss_weights[2],'Adv G': advLoss.item(),'Adv Dloss':DLoss.item(),'Adv D - Real' : D_real_loss.item(),'Adv D - Fake' : D_fake_loss.item(), 'Train D': train_D, 'Prctg of train set': progress})
                    else:
                        pbar.set_postfix(**{'FullLoss': FullLoss.item(), 'ImL2': ImL2.item(), 'ImL1': ImL1.item(),
